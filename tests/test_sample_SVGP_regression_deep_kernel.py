...
def test_deep_kernel():
    ...
    deepkernel = DeepKernel((2, 1),
                    ...
                    batch_size=200, 
                    cnn=cnn)
    deepkernel_SVGP_model = SampleSVGP(
            deepkernel,
            gpflow.likelihoods.Gaussian(),
            inducing_variable=inducing_variables,
            num_data=len(X),
    ...
    print(trained_elbo, trained_deepkernel_elbo)
    np.testing.assert_allclose(trained_elbo, trained_deepkernel_elbo, rtol=0.05, atol=0.0)
